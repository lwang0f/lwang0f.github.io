...
For a large course table, keep the source CSV in `files/` with an ASCII filename and show a compact summary table before the full detail table. Put the complete data inside a collapsible `<details class="teaching-grade-table-details">` block, with each table inside a `<div class="teaching-grade-table-wrapper">`. Use the `teaching-grade-table` class so the table remains readable on desktop and can scroll horizontally on mobile. Preserve multi-row headers, wrap header labels rather than data cells, and keep identity fields left-aligned while numeric fields are right-aligned. The CSV is the source of truth: the web presentation may round numeric scores to whole numbers and normalize tiny scientific-notation calculation residues to `0`, but must not overwrite the original CSV precision. Keep a short note and a link to the raw CSV beside each table caption.
